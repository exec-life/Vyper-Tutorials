...
# Note: because Vyper does not allow for dynamic arrays, we have limited the number
# of bids that can be placed by one address to 128 in this example
...
# `_beneficiary`
@external
def __init__(_beneficiary: address, _bidding_time: uint256, _revealTime: uint256):
    self.beneficiary = _beneficiary
    self.biddingEnd = block.timestamp + _bidding_time
...
def bid(_blindedBid: bytes32):
    ...
    # Check the payer hasn't already placed the maximum number of bids
...
def placeBid(bidder: address, _value: uint256) -> bool:
    # If bid is less than the highest bid, bid fails
...
# invalid bids and all bids except for the totally highest.
@external
...
def withdraw():
    ...
    if(pendingAmount > 0):
        # If so, set pending return to zero to prevent recipient from calling
...
# End the auction and give the highest bid to the beneficiary
@external
def auctionEnd():
